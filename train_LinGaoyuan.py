...
def train(args):
    ...
    # 创建viser数据保存目录
    viser_data_dir = os.path.join(out_folder, "viser_data")
    os.makedirs(viser_data_dir, exist_ok=True)

    # 启动viser viewer
    viewer_script = os.path.join(os.path.dirname(__file__), "viewer/run_viewer.py")
    viewer_process = subprocess.Popen([
        sys.executable,
        viewer_script,
        "--data_dir", viser_data_dir,
        "--port", str(args.viser_port)
    ])
    ...
    try:
        ...
        for epoch in range(args.max_epochs):
            ...
            for train_data in train_loader:
                ...
                # 生成viser数据
                viser_data = generate_viser_data(ray_batch, ret)
                
                # 保存数据
                if global_step % args.i_viser == 0:  # 每隔一定步数保存一次
                    data_path = os.path.join(viser_data_dir, f"viser_data_{global_step:06d}.npz")
                    np.savez(data_path, **viser_data)
...
def generate_viser_data(ray_batch, ret):
    """生成viser可用的3D数据"""
    # 获取深度图和RGB信息
    depth_map = ret["outputs_coarse"]["depth"].detach().cpu().numpy()
    rgb_map = ret["outputs_coarse"]["rgb"].detach().cpu().numpy()
    
    # 获取相机参数
    camera = ray_batch["camera"]
    ray_o = ray_batch["ray_o"].detach().cpu().numpy()
    ray_d = ray_batch["ray_d"].detach().cpu().numpy()
    ...
    for i in range(H):
        for j in range(W):
            ...
            if depth > 0:  # 过滤无效深度
                # 计算3D点坐标
                point_3d = ray_o[i*W + j] + depth * ray_d[i*W + j]
                points_3d.append(point_3d)
                colors.append(rgb_map[i, j])
